...
void insereal(aluno **point)
{
	...
	else
	{
		...
		scanf("%d", &codigo);
		ajuda->codigo=codigo;
		printf("Escreva o cpf do aluno(somente numeros):");
		scanf("%lld", &cpf);
		ajuda->cpf=cpf;
...
void inseredis(disciplina **point)
{
	...
	else
	{
		...
		ajuda->pessoa=NULL;//facoo o ponteiro de alunos apontar pra NULL, pra quando inserir uma disciplina pra esse aluno, o ponteiro de proximo ficar apontando pro null, e encadear a lista
		ajuda->prox=*point;
		*point=ajuda;
	}
}
    	...
    	while(ch!='^')
    	{
			...
			if(ch =='/' && j==2 && k==0)
    		{
				...
				*al = aln;
				...
				{
					suport=*al;
					ndis2 = (nomedis*)malloc(sizeof(nomedis));
					suport->materia = NULL;
					break;
				}
			}
